...
def traj_segment_generator(agent):
    while True:
        agent.run()
        #print("Getting dataset")
        data = agent.get_dataset()
        #print("Got dataset")
...
class TrpoTrainer():
    def __init__(self, agent, env, policy_func, *,
            timesteps_per_batch,
            max_kl, cg_iters,
            gamma, lam,
            entcoeff=0.0,
    ...
    def train(self, agent, model_name):
        nworkers = MPI.COMM_WORLD.Get_size()
        rank = MPI.COMM_WORLD.Get_rank()
        seg_gen = traj_segment_generator(agent)
