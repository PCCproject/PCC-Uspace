...
// Factor to penalize sending by
const float kSendingCoefficient = -0.1;
}  // namespace

float PccSubRateUtilityCalculator::CalculateUtility(MonitorInterval& cur_mi) {

  float throughput = cur_mi.GetObsThroughput();
  float thpt_mbps = throughput / kBitsPerMegabit;
  ...
  float sending_contribution = kSendingCoefficient * pow(sending_rate_mbps, kExponent);
  float thpt_contribution = pow(thpt_mbps, kExponent);
  
  float utility = thpt_contribution
                + sending_contribution
                + loss_contribution
                + rtt_contribution;
